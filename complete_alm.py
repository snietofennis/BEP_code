...
###############
    ...
    circuit.L('Income',            'N011', 'N007',            1.5   @ u_H, raw_spice = 'IC=10')
    circuit.L('Consumption',       'N007', 'N012',            1     @ u_H, raw_spice = 'IC=10')
    # ─── ideal coupling: KH1 L_Consumption & L_Income, KH2 L_Income & Savings
        
    # circuit.raw_spice += '\nKHK1 LConsumption LIncome -0.2\n'
    # circuit.raw_spice += '\nKHK2 LIncome      LSavings  -0.8\n'

    circuit.K('H1', 'LConsumption', 'LIncome',  -0.2)
    circuit.K('H2', 'LIncome',      'LSavings', -0.8)
    # after your existing K calls
    circuit.K('H3', 'LConsumption', 'LSavings', 0.0)
...
"""
    ...
    circuit.B('Spread',                  circuit.gnd, circuit.gnd,
                current_expression='0.005-0.1*I(BSpread_Err2)+{Ki}*I(BSpread_Err2_output)')
...
+ out_lower_limit=-1e12 out_upper_limit=1e3
+ limit_range=1e-9 out_ic=0)
R_FTP_Err2 SFTP_Err2_idt 0 1Meg
"""
    ...
    circuit.B('FTP_Rate',                circuit.gnd, circuit.gnd, 
                current_expression='0.025-(2*0.06*I(BFTP_Err2)+1.5*0.003*I(BFTP_Err2_output))')
...
def run_transient(circuit, step_time=1 @ u_s, end_time=1500 @ u_s):
    simulator = circuit.simulator()
    return simulator.transient(step_time=step_time, end_time=end_time, use_initial_condition=True)


def plot_node_voltage(circuit, analysis, node_control='N005',node_controlled='N013', node_error='v_tap'):
    time = analysis.time
