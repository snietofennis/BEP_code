...
def steps(circuit, name, rates, times):
    ...
    for interest_rate in rates:
        if times == None:
            ...
            width = times[i] - times[i-1] if i > 0 else 20
...
# Control parameters
    ...
    control_debt_equity=False, # Controls spread
    control_tier_1=False, # Controls spread

    use_preset_FTP = False,
    preset_FTP_rates = [1, 1, 3,2, 1],
    preset_FTP_times = [20,30, 44, 50, 80],  # can be None

    use_preset_spread = False,
    preset_spread = [0.005, 0.005, 0.01, 0.01, 0.005],
    preset_spread_times = [20, 30, 44, 50, 80],  # can be None
...
#Economic parameters
    ...
    use_preset_Trate = False,
    preset_T_rates = [0.025, 0.025, 0.03, 0.03, 0.025],
    preset_T_times = [20, 30, 44, 50, 80],  # can be None 
...
# Simulation parameters
    ...
    circuit.B('Incentive_to_Borrow',     'Incentive_to_Borrow', circuit.gnd,
                voltage_expression='-3*(I(BLoan_Interest_Rate)-I(BT_rate)-0.020)') 
...
############# INTEGRATING NII
    ...
    if use_preset_Trate:
        steps(circuit, 'Trate', preset_FTP_rates, preset_FTP_times)
    ...
    if control_loan_desposit == False and use_preset_FTP == False:
...
def run_transient(circuit, step_time=dt @ u_s, end_time=300 @ u_s):
    simulator = circuit.simulator()
    return simulator.transient(step_time=dt, end_time=end_time, use_initial_condition=True)


def plotting(circuit, analysis, plot_1 = 'N013', plot_2 ='v_bloan-to-deposit_ratio1', plot_3 ='BIncentive_to_Borrow'):
    time = analysis.time
...
def run_recommend_discrete(time, u_continuous, Ts):
    ...
    circuit = ALM(use_preset_FTP=True, preset_FTP_rates = rates, preset_FTP_times= times_zoh)
...
def main():
    ...
    circuit = ALM(control_loan_desposit=True)
